...
class SearchFeedbackTelegramCommandFunctionalTest extends TelegramCommandFunctionalTestCase
{
    ...
    public function onStartStepDataProvider(): Generator
    {
        ...
        yield sprintf('start_step_as_%s', 'text') => [
            'command' => ChooseFeedbackActionTelegramConversation::getSearchButton($this->tg)->getText(),
    ...
    public function testCancelSuccess(int $step, bool $change = null): void
    {
        $this->typeCancel(
            ...
            shouldSeeReply: [
                $this->trans('reply.icon.upset') . ' ' . $this->trans('reply.search.canceled'),
    ...
    ): void
    {
        ...
        if ($count === 0) {
            $shouldSeeReply[] = $this->trans('reply.icon.upset') . ' ' . $this->trans('reply.search.empty_list', ['search_term' => $state->getSearchTerm()->getText()]);
        } else {
            ...
            $shouldSeeReply[] = $this->trans('reply.icon.ok') . ' ' . $this->trans('reply.search.summary', ['count' => $count]);
        }
